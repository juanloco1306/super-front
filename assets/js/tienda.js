...
function addToCartClicked(event) {
  ...
  const item = button.closest('.gift__card');

  const itemTitle = item.querySelector('.gift__title').textContent;
  const itemPrice = item.querySelector('.gift__price').textContent;
  const itemImage = item.querySelector('.gift__img').src;
...
function updateShoppingCartTotal() {
  ...
  shoppingCartItems.forEach((shoppingCartItem) => {
    ...
    const shoppingCartItemPrice = Number(
      shoppingCartItemPriceElement.textContent.replace('MXN', '')
    );
